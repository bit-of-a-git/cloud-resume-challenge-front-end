...
resource "aws_cloudfront_response_headers_policy" "security_headers_policy" {
  ...
  security_headers_config {
    ...
    content_security_policy {
      content_security_policy = "frame-ancestors 'none'; default-src 'self' https://fthtevm397.execute-api.eu-west-1.amazonaws.com/dev; img-src 'self'; script-src 'self'; style-src 'self' https://fonts.googleapis.com https://cdnjs.cloudflare.com; font-src 'self' https://fonts.gstatic.com https://cdnjs.cloudflare.com; object-src 'none'"
      override                = true
    }
  }
}
